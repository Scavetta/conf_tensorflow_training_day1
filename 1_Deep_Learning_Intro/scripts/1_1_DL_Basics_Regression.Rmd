...
`verbose = TRUE`, by defaule, so it prints all the values to the screen. That's fine, but we can also save the results to an object as it trains:
...
We can manually use the weights and biases to calculate the result to full understand what the model is doing. Let's see if we can undertand how we went from the firs test instance to the result.
