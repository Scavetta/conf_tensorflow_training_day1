...
The target, i.e. response, variable is continuous, but for this example, we'll pretend that it's binary so that we can see how to handle a binary classification.
...
The network begins to overfit after about 30 - 40 epochs. We'll discuss this in more detail in the next session.
...
data.frame(target = actual,
  ...
  scale_x_continuous("Actual Target", breaks = 0:1, limits = 0:1, labels = c("Negative", "Positive")) +
  scale_y_continuous("Prediction", breaks = 0:1, limits = 0:1, labels = c("Negative", "Positive")) +
