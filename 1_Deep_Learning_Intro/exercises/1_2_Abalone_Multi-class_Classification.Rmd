---
title: "Your Turn: Abalone -- Single-label, Multi-class Classification"
...
## New functions in this section:

In regression and binary classification, we didn't do anything to our labels (indeed, you should recall that logistic regression predicts a probabiliby [0,1])
...
|:---------|:-----|
| `to_categorical()` | Vectorizes over an integer.  |


In the compile step, we need to change the loss function to deal with this. Two loss functions: 

| Loss Function | Description |
|:---------|:-----|
| `sparse_categorical_crossentropy` | Use when targets (i.e. labels) are encoded as integers |
| `cateogircal_crossentropy` | Use when targes (i.e. labels) have been vectorized with `to_categorical()` |
...
train_data <- read_csv(paste0(data_fir, "abalone_data.csv"))
train_labels <- read_csv(paste0(data_fir, "abalone_labels_cat.csv"))
