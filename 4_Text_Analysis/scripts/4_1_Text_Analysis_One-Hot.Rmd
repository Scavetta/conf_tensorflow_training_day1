...
We'll again use one-hot encoding:
...
In one-hot noding, we only get 1s and 0s. The position of the 1s corresponds to the indices above:
...
Our accuracty is `r metrics$acc` and the error rate, i.e. incorrect calling, is `r 1 - metrics$acc`.

### Predictions

Predict classes will give us the integer values from the test data:
